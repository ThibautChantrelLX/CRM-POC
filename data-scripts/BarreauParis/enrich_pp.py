...
INPUT      = os.path.join(ROOT, 'output', 'BarreauParis', 'extraction_barreau_paris_pp.csv')
OUTPUT     = INPUT  # mise à jour sur place
CHECKPOINT = os.path.join(ROOT, 'output', 'BarreauParis', '.enrich_pp_checkpoint.json')
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Enrichissement Structure(s) des PP via GetAvocatByCnbf.')
    parser.add_argument('--token', help='Bearer token (sinon BARREAU_PARIS_BEARER_TOKEN).')
    parser.add_argument('--workers', type=int, default=5, help='Nombre de workers parallèles (défaut: 5).')
...
def fetch_detail(cnbf: str, token: str, session: requests.Session) -> Optional[dict]:
    ...
    try:
        resp = session.get(DETAIL_URL, headers=build_headers(token),
                           params={'cnbf': cnbf}, timeout=15)
        ...
        def join_field(lst):
            ...
            for item in lst:
                if isinstance(item, str) and item:
                    ...
                    v = item.get('libelle') or item.get('LIBELLE') or item.get('nom') or ''
                    if v:
                        parts.append(v)
            return ' | '.join(parts)
...
def main() -> None:
    ...
    with requests.Session() as session:
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            futures = {
                executor.submit(fetch_detail, r['ID Contact'], token, session): r['ID Contact']
                for r in todo
            }
            for future in as_completed(futures):
                ...
                detail = detail or {'structures': '', 'activites': '', 'specialites': '', 'date_serment': ''}
                ...
                if processed % SAVE_EVERY == 0:
                    ...
                    found_s = sum(1 for v in checkpoint.values() if isinstance(v, dict) and v.get('structures'))
                    found_a = sum(1 for v in checkpoint.values() if isinstance(v, dict) and v.get('activites'))
                    found_sp = sum(1 for v in checkpoint.values() if isinstance(v, dict) and v.get('specialites'))
                    print(f'  [{processed:6d}/{len(todo)}]  struct: {found_s}  act: {found_a}  spec: {found_sp}  '
