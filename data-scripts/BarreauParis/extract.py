...
CRITERIA_URL  = 'https://apiresteannuairemiddleware.avocatparis.org/api/GetAllCriteriaList'
CRITERIA_FILE = os.path.join(ROOT, 'criteria.json')

DEFAULT_OUTPUT  = os.path.join(ROOT, 'output', 'BarreauParis', 'extraction_barreau_paris.csv')
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Extraction Barreau de Paris — contacts.')
    parser.add_argument('--token', help='Bearer token (sinon BARREAU_PARIS_BEARER_TOKEN).')
    parser.add_argument('--nom', default='', help='Filtre par nom (mode ciblé, min 2 chars).')
    parser.add_argument('--act', type=int, default=None, help='Code activité (mode ciblé).')
    parser.add_argument('--spec', type=int, default=None, help='Code spécialité (mode ciblé).')
    parser.add_argument('--page-size', type=int, default=30, help='Résultats par page (défaut: 30).')
    parser.add_argument('--max-pages', type=int, default=0, help='Pages max par requête (0 = toutes).')
    parser.add_argument('--details', action='store_true', help='Enrichir Case et Date serment via GetAvocatByCnbf.')
    parser.add_argument('--list-criteria', action='store_true', help='Afficher les critères disponibles.')
    parser.add_argument('--criteria-output', default='', help='Sauvegarder les critères dans ce fichier JSON.')
...
def assemble_row(item: dict) -> dict:
    ...
    adresse        = clean_value(item.get('adresse_paris', '')) or clean_value(item.get('adresse_Etranger', ''))
...
def fetch_page_raw(session: requests.Session, token: str, nom: str,
                   act: Optional[int], spec: Optional[int],
    ...
    if spec is not None:
        params['spec'] = spec
    resp = session.get(SEARCH_URL, headers=build_headers(token), params=params, timeout=30)
    ...
    data = resp.json()
    total = data.get('metadata', {}).get('totalResult', 0) if isinstance(data, dict) else 0
...
def fetch_all_pages(session: requests.Session, token: str, nom: str,
                    act: Optional[int], spec: Optional[int],
                    page_size: int, max_pages: int) -> Tuple[dict, int]:
...
def extract_by_name_prefixes(session: requests.Session, token: str,
                             page_size: int, max_pages: int,
                             all_rows: dict, output: str) -> None:
    ...
    print(f'  {len(bigrams)} bigrammes au total (seuil subdivision : {CAP_THRESHOLD})...')

    for i, bigram in enumerate(todo, start=1):
        rows, total = fetch_all_pages(session, token, bigram, None, None, page_size, max_pages)
        before = len(all_rows)
        ...
        if i % 25 == 0 or new > 50:
            ...
            print(f'  [{i:3d}/{len(todo)}]  {bigram}  total={total:4d}  +{new:4d} nouveaux'
    ...
    if capped_todo:
        ...
        for bigram in capped_todo:
            for letter in LETTERS:
                ...
                if trigram in done:
                    continue
                rows, _ = fetch_all_pages(session, token, trigram, None, None, page_size, max_pages)
                before  = len(all_rows)
                all_rows.update(rows)
                new = len(all_rows) - before
                mark_done(trigram)
...
def extract_full(session: requests.Session, token: str,
                 page_size: int, max_pages: int, output: str) -> dict:
    ...
    for s in specs:
        ...
        if key in done:
            continue
        rows, total = fetch_all_pages(session, token, '', None, code, page_size, max_pages)
        before  = len(all_rows)
        all_rows.update(rows)
        new = len(all_rows) - before
        mark_done(key)
        save_csv(list(all_rows.values()), output, False)
        print(f'      spec {code:3}  {libelle[:45]:45s}  {total:4d} résultats  +{new} nouveaux')
    ...
    for a in acts:
        ...
        if key in done:
            continue
        rows, total = fetch_all_pages(session, token, '', code, None, page_size, max_pages)
        before  = len(all_rows)
        all_rows.update(rows)
        new = len(all_rows) - before
        mark_done(key)
        save_csv(list(all_rows.values()), output, False)
        print(f'      act  {code:3}  {libelle[:45]:45s}  {total:4d} résultats  +{new} nouveaux')
...
def print_criteria(criteria: Dict[str, Any], output_file: str = '') -> None:
    ...
    for key in ('specialite', 'activite', 'mandat', 'langue', 'fonctions'):
        ...
        for item in values:
            if isinstance(item, dict):
                print(f"  {item.get('CODE', '')} = {item.get('LIBELLE', item.get('label', ''))}")
            else:
                print(f'  {item}')
    if output_file:
...
def fetch_detail(session: requests.Session, token: str, cnbf: str) -> dict:
    resp = session.get(DETAIL_URL, headers=build_headers(token), params={'cnbf': cnbf}, timeout=30)
    resp.raise_for_status()
...
def enrich_with_details(rows: list, session: requests.Session, token: str) -> None:
    ...
    for i, row in enumerate(rows, start=1):
        ...
        row['Date serment'] = extract_field(lower, ['DateSerment', 'dateSerment', 'date_de_serment', 'date'], '')
        if i % 50 == 0:
        ...
        if targeted:
            ...
            try:
                all_rows, _ = fetch_all_pages(
                    session, token, args.nom, args.act, args.spec,
            ...
            try:
                all_rows = extract_full(session, token, args.page_size, args.max_pages, args.output)
            except TokenExpiredError as exc:
                raise SystemExit(f'\n⚠️  {exc}')
            except Exception as exc:
                raise SystemExit(f'Erreur API : {exc}')
