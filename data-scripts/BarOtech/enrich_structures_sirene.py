...
def extract_fields(nom, data):
    if not data:
        return {
            'raison_sociale': nom,
            'siret': '', 'siren': '',
            'type_structure': '', 'categorie_entrepri': '',
            'secteur_activite': '', 'site_web': '',
            'source_origine': SOURCE,
            'rue': '', 'complement_adres': '',
    ...
    return {
        'raison_sociale': data.get('nom_complet') or data.get('nom_raison_sociale') or nom,
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            if processed % 200 == 0:
                ...
                print(f"  {processed:5d}/{len(todo)} | {rate:.1f} req/s | ~{remaining/60:.0f} min | trouvés: {found}")
